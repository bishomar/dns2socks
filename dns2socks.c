...
#define DEFAULT_DNS_SERVER "8.8.4.4"
#define DEFAULT_REMOTE_DNS_PORT "53"
#define DEFAULT_LISTEN_IP "127.0.0.1"
#define DEFAULT_LOCAL_DNS_PORT "5335"
...
int main(int iArgCount, char** szaArgs)
{
	...
	} saAddresses[3]=
	{
		...
		{ &g_sDnsSrvAddr, DEFAULT_DNS_SERVER, DEFAULT_REMOTE_DNS_PORT, "DNS server", 0 },
		{ &sAddr, DEFAULT_LISTEN_IP, DEFAULT_LOCAL_DNS_PORT, "listening", AI_PASSIVE }	//AI_PASSIVE for "getaddrinfo" as we use it for "bind"
	};
	...
	for(pszCurArg=szaArgs+1; --iArgCount; ++pszCurArg)
	{
		...
		OutputFatal("\nDNS2SOCKS tunnels DNS requests via SOCKS5 and caches the answers.\n\n\n"
			...
			DEFAULT_DNS_SERVER, DEFAULT_REMOTE_DNS_PORT,
			DEFAULT_LISTEN_IP, DEFAULT_LOCAL_DNS_PORT);
		return 1;
	}
